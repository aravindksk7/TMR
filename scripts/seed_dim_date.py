...
Usage:
    python scripts/seed_dim_date.py                # uses dates from .env defaults
    python scripts/seed_dim_date.py --start 2015-01-01 --end 2035-12-31
"""
...
def _build_row(d: date) -> tuple:
    ...
    # Simple fiscal year: same as calendar year (adjust if needed)
    fiscal_year    = d.year
...
def seed(
    ...
    """Insert dim_date rows from *start* to *end* inclusive. Returns row count."""
    cursor = conn.cursor()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Seed dim_date table")
    ...
    with build_connection(dsn) as conn:
